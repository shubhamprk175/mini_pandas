...
class TestAggregation:
    ...
    def test_min(self):
        df_result = df1.min()
        df_answer = mpd.DataFrame({'a': np.array(['a'], dtype='O'),
                                   'b': np.array([5]),
                                   'c': np.array([np.nan])})
        assert_df_equals(df_result, df_answer)
    ...
    def test_max(self):
        df_result = df1.max()
        df_answer = mpd.DataFrame({'a': np.array(['c'], dtype='O'),
                                   'b': np.array([11]),
                                   'c': np.array([np.nan])})
        assert_df_equals(df_result, df_answer)
    ...
    def test_mean(self):
        df_result = df1.mean()
        df_answer = mpd.DataFrame({'b': np.array([8.]),
                                   'c': np.array([np.nan])})
        assert_df_equals(df_result, df_answer)
    ...
    def test_sum(self):
        df_result = df1.sum()
        df_answer = mpd.DataFrame({'a': np.array(['abc'], dtype='O'),
                                   'b': np.array([24]),
                                   'c': np.array([np.nan])})
        assert_df_equals(df_result, df_answer)
    ...
    def test_var(self):
        df_result = df1.var()
        df_answer = mpd.DataFrame({'b': np.array([b1.var()]),
                                   'c': np.array([np.nan])})
        assert_df_equals(df_result, df_answer)

    def test_std(self):
        df_result = df1.std()
        df_answer = mpd.DataFrame({'b': np.array([b1.std()]),
                                   'c': np.array([np.nan])})
        assert_df_equals(df_result, df_answer)

    def test_all(self):
        df_result = df2.all()
        df_answer = mpd.DataFrame({'a': np.array([False]),
                                   'b': np.array([True]),
                                   'c': np.array([False])})
        assert_df_equals(df_result, df_answer)
    ...
    def test_any(self):
        df_result = df2.any()
        df_answer = mpd.DataFrame({'a': np.array([True]),
                                   'b': np.array([True]),
                                   'c': np.array([True])})
        assert_df_equals(df_result, df_answer)
    ...
    def test_argmax(self):
        ...
        df_answer = mpd.DataFrame({'a': np.array([2]),
                                   'b': np.array([0]),
                                   'c': np.array([1])})
        assert_df_equals(df_result, df_answer)
    ...
    def test_argmin(self):
        ...
        df_answer = mpd.DataFrame({'a': np.array([0]),
                                   'b': np.array([1]),
                                   'c': np.array([1])})
        assert_df_equals(df_result, df_answer)
...
df3 = mpd.DataFrame({'a': a3, 'b': b3, 'c': c3})

a4 = np.array(['a', 'a', 'c'], dtype='O')
b4 = np.array([11, 5, 5])
c4 = np.array([3.4, np.nan, 3.4])
df4 = mpd.DataFrame({'a': a4, 'b': b4, 'c': c4})
...
class TestOtherMethods:
    ...
    def test_isna(self):
        df_result = df3.isna()
        df_answer = mpd.DataFrame({'a': np.array([False, True, False]),
    ...
    def test_rename(self):
        ...
        df_answer = mpd.DataFrame({'A': a4, 'b': b4, 'C': c4})
        assert_df_equals(df_result, df_answer)

    def test_drop(self):
        df_result = df4.drop(['a', 'b'])
        df_answer = mpd.DataFrame({'c': c4})
        assert_df_equals(df_result, df_answer)


a42 = np.array([-11, 5, 3])
b42 = np.array([3.4, 5.1, -6])
df42 = mpd.DataFrame({'a': a42, 'b': b42})
...
class TestNonAgg:
    ...
    def test_abs(self):
        df_result = df42.abs()
        df_answer = mpd.DataFrame({'a': np.abs(a42), 'b': np.abs(b42)})
        assert_df_equals(df_result, df_answer)

    def test_cummin(self):
        df_result = df42.cummin()
        df_answer = mpd.DataFrame({'a': np.array([-11, -11, -11]),
                                   'b': np.array([3.4, 3.4, -6])})
        assert_df_equals(df_result, df_answer)

    def test_cummax(self):
        df_result = df42.cummax()
        df_answer = mpd.DataFrame({'a': np.array([-11, 5, 5]),
                                   'b': np.array([3.4, 5.1, 5.1])})
        assert_df_equals(df_result, df_answer)

    def test_cumsum(self):
        df_result = df42.cumsum()
        df_answer = mpd.DataFrame({'a': np.array([-11, -6, -3]),
                                   'b': np.array([3.4, 8.5, 2.5])})
        assert_df_equals(df_result, df_answer)

    def test_clip(self):
        df_result = df42.clip(0, 4)
        df_answer = mpd.DataFrame({'a': np.array([0, 4, 3]),
                                   'b': np.array([3.4, 4, 0])})
        assert_df_equals(df_result, df_answer)

    def test_round(self):
        df_result = df42.round(0)
        df_answer = mpd.DataFrame({'a': np.array([-11, 5, 3]),
                                   'b': np.array([3, 5, -6])})
        assert_df_equals(df_result, df_answer)
    ...
    def test_diff(self):
        ...
        df_answer = mpd.DataFrame({'a': np.array([np.nan, 16, -2]),
                                   'b': np.array([np.nan, 1.7, -11.1])})
        assert_df_equals(df_result, df_answer)

    def test_pct_change(self):
        df_result = df42.pct_change(1)
        df_answer = mpd.DataFrame({'a': np.array([np.nan, 16 / -11, -2 / 5]),
...
df6 = mpd.DataFrame({'a': a6, 'b': b6})

a7 = np.array(['b', 'a', 'a', 'a', 'b'])
b7 = np.array([3.4, 5.1, 2, 1, 6])
df7 = mpd.DataFrame({'a': a7, 'b': b7})
...
df8 = mpd.DataFrame({'a': a8, 'b': b8, 'c': c8})
...
df_string = mpd.DataFrame({'movie': movie, 'num': num})
...
df_emp = mpd.read_csv('data/employee.csv')
